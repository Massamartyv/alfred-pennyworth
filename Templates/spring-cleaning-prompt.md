...
5. **Ventures** — `Context/Spheres/System/Entrepreneurship/`. Verify every active venture mirrors the nine-department filing standard – Foundation, Administration, Finances, Business Development, Marketing & Sales, Operations, Product Development, Human Resources, Knowledge Base – and matches the bins in the Five Points Google Drive. Check the `New Venture/` template matches current standard. Inventory client folders under each venture and flag any that are incomplete, mislabeled, or outside the standard. Verify each venture has `Agents/integrations.md` and `Agents/agent-guidelines.md` if warranted.
