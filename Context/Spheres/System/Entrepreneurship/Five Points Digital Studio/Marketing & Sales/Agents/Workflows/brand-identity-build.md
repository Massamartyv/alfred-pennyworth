---
file_type: workflow
department: Marketing & Sales
workflow_name: Brand Identity Build
venture: Five Points Digital Studio
methodology: The Manor Protocol
last_updated: 2026-09-10
---

# Brand Identity Build

Full brand identity for a new client. The most complex workflow in Marketing & Sales – activates all five specialist roles and runs the complete Manor Protocol lifecycle.

---

## Reconnaissance

**Role:** Researcher
**Crew:** Researcher

- Gather client discovery brief from Operations/Clientele/{Client}/Onboarding/client-brief.md
- Research the client's industry, competitors and audience
- Collect visual references, mood boards, tonal examples
- Identify cultural signals and trends relevant to the client's space
- Surface existing brand assets, history, and what the client has tried before

**Output:** Reconnaissance brief – landscape of possibility, not answers.

---

## Direction

**Role:** Brand Strategist
**Crew:** Creator

- Synthesise the reconnaissance into a creative brief
- Define brand positioning: who, what, why, how, against whom
- Establish voice attributes: personality, tone range, language to embrace and avoid
- Set visual direction: colour territory, typography direction, spatial philosophy
- Define the brand's relationship to culture – what it references, what it avoids

**Output:** Creative brief with positioning, voice attributes and visual direction.

**Hard gate: human approval required before proceeding.**

---

## Execution

**Roles:** Art Director, Copywriter
**Crew:** Creator

### Art Director
- Develop colour palette (primary, secondary, accent)
- Select typography system (headings, body, accent)
- Create spatial and layout principles
- Establish photography and imagery direction
- Design brand mark direction (if in scope)

### Copywriter
- Write brand voice guide with examples
- Develop messaging hierarchy (headline, subhead, body, CTA)
- Create sample copy in brand voice across contexts
- Write tagline options (if in scope)

**Output:** Brand identity system – visual and verbal, interconnected.

---

## Critique

**Role:** Editor
**Crew:** Reviewer:Scrutiny

Evaluate against the four non-negotiable criteria (see `Criteria/quality-rubric.md`):

1. **Brand alignment** – Does the identity feel coherent? Do visual and verbal reinforce each other?
2. **Originality** – Is it distinctive in the client's market? Does it avoid category cliches?
3. **Craft and finish** – Are colour values precise? Is typography properly scaled? Is copy rhythm intentional?
4. **Emotional resonance** – Does the identity evoke the intended feeling?

**Output:** Critique report with pass/fail on each criterion and specific revision notes for any failures.

**Hard gate: human approval required before proceeding.**

---

## Release

**Role:** Head of Marketing & Sales (oversight)
**Crew:** Broadcaster

- Package deliverables for client presentation
- Prepare brand guidelines document
- Hand off to Operations for client delivery
- Archive brand assets in Operations/Clientele/{Client}/Brand Assets/

**Output:** Complete brand identity package, delivered.
