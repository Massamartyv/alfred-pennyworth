---
file_type: department_index
department: Creative
venture: Marty Gras
last_updated: 2026-04-22
---

# Creative

Editorial direction, voice, brand system and content product definitions. What Marty Gras sounds like, looks like and feels like. The Creative studio holds the editorial standard that every piece of output is measured against.

## Structure

```
Creative/
  _index.md              -- This file
  Content Products/      -- Definitions and formats for each product (Epiphany, podcast, social)
  Editorial/             -- Pillars, themes, seasonal calendars, series concepts
  Products/              -- Physical product and merchandise design briefs, e.g. the Marty Bag
  Agents/                -- Studio-specific agents, workflows, criteria
    _index.md            -- Specialist roster and workflow registry
    Workflows/           -- Named Manor Protocol sequences
    Criteria/            -- Quality rubrics and evaluation standards
```

## Scope

### What belongs here

- Content product definitions and formats (newsletter, podcast, social)
- Editorial direction and theme planning
- Content pillars and series concepts
- New product ideation and development
- Brand voice enforcement across channels
- Visual direction for the brand

### What does not belong here

- Production workflows and SOPs – those go in Production
- Distribution and scheduling – those go in Operations
- Audience growth tactics – those go in Growth

## Content Products

| Product | Format | Platform | Cadence | Status |
|---|---|---|---|---|
| Epiphany | Newsletter | Substack | To be set | In development |
| Marty Gras Podcast | Long-form audio, video | Podcast platforms, YouTube | To be set | In development |
| Social content | Short-form visual and written | Instagram, TikTok, LinkedIn, Threads, X, Reddit | Daily to weekly | Active via personal brand |

### Product Definitions

**Epiphany Newsletter** – Cultural commentary, curated ideas and perspective pieces delivered via Substack. The written voice of the Architect of Vibe. Subscribers receive a window into how culture, creativity and entrepreneurship intersect.

**Marty Gras Podcast** – Long-form conversations and solo episodes exploring culture, creativity, personal development and the entrepreneurial journey. Produced with ElevenLabs for audio excellence. Video versions for YouTube.

**Social Content** – Platform-native cultural curation. Visual storytelling on Instagram, thought leadership on LinkedIn, short-form commentary on TikTok and Threads. Each platform gets content shaped to its native language, not cross-posted identically.

## Key Context

- **Voice foundation:** `Context/personal-brand-identity.md`
- **Brand profile:** `Foundation/brand-fingerprint.md` at venture root (retired `Context/martyv-identity.md` archived 2026-07-23, unique content inherited there)
- **Aesthetic sensibility:** `Context/creative-director.md`
- **Visual system:** `brand-fingerprint.md` at venture root

## Agent Instructions

- All content product changes require approval before taking effect.
- Editorial direction decisions are Tier 3 (human approval required).
- Brand voice enforcement is a continuous background check on all output.
- Load brand voice files before producing any audience-facing copy.

### Token Budget Defaults

| Task | Tier |
|---|---|
| Single-post brand voice review | Light |
| Editorial briefs, content pillar updates, product concept docs | Standard |
| Full editorial strategy, new content product launches | Heavy |
