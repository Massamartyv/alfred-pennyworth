...
| `notion-personal` | `@notionhq/notion-mcp-server` (npx) | Personal Notion workspace – the working route since the OAuth enhanced connector died 12 July; also serves the Martywood standing exception. **First recorded in this registry 2026-08-11** – it was registered in `.mcp.json` but undocumented here | `NOTION_PERSONAL_TOKEN` |
...
| `elevenlabs` | `elevenlabs-mcp` (uvx, official) | Personal – voice production for the Marty Gras Conversation lane. **Registered 2026-07-10; awaiting key** | `ELEVENLABS_API_KEY` (wrapper also sets non-secret `ELEVENLABS_MCP_BASE_PATH` to `.working/elevenlabs/`) |
...
**Every sovereign venture holds its own Notion workspace and its own integration token.** One standing exception: Martywood – the personal brand house, named Marty Gras until 2026-09-02 – is the personal media identity and routes to the personal workspace permanently, by ruling rather than deferral. Sub-brands inherit the workspace of the parent venture – Paradigm Farms does not provision its own.
...
| Martywood | Personal workspace, by standing exception | `notion-personal` | Live with the personal server |
...
- **One Notion workspace per sovereign venture, ruled 2026-08-08 (The Junction).** Replaces the two-path personal/Five Points rule. Martywood is the single standing exception. Full table above. The project CLAUDE.md already carried the principle – "Venture A's Notion workspace is not Venture B's Notion workspace" – so this is execution catching up with doctrine rather than new policy.
- **Four venture integration registries created 2026-08-08 (The Junction).** Paradigm, Lillie and Lynette, Atlas and Athena had none; only Five Points and Martywood – then Marty Gras – did. The root cause was the `New Venture/` template, which shipped the sentence "No `Agents/integrations.md` exists until configured" in three separate files instead of shipping a registry to fill in. Every venture copied from it inherited an absence. The template now ships a blank registry carrying a three-row provisioning checklist – Notion workspace, Pennyone pipeline, payment rail – and the three deferral sentences are gone.
